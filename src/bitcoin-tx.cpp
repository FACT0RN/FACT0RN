...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " factorn-tx utility version " + FormatFullVersion() + "\n";
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\n"
                "Usage:  factorn-tx [options] <hex-tx> [commands]  Update hex-encoded factorn transaction\n"
                "or:     factorn-tx [options] -create [commands]   Create hex-encoded factorn transaction\n"
                "\n";
